_base_ = [
    '../_base_/datasets/mpii_keypoint.py', '../_base_/default_runtime.py'
]
model = dict(
    type='opera.PETR',
    backbone=dict(
    ...
    bbox_head=dict(
        ...
        num_keypoints=16, # From 17 to 16 in MPII
        in_channels=2048,
        ...
        loss_oks=dict(type='opera.OKSLoss', num_keypoints=16, loss_weight=2.0),
        loss_hm=dict(type='opera.CenterFocalLoss', loss_weight=4.0),
        loss_kpt_refine=dict(type='mmdet.L1Loss', loss_weight=80.0),
        loss_oks_refine=dict(type='opera.OKSLoss', num_keypoints=16, loss_weight=3.0)),
    train_cfg=dict(
        assigner=dict(
            type='opera.PoseHungarianAssigner',
            cls_cost=dict(type='mmdet.FocalLossCost', weight=2.0),
            kpt_cost=dict(type='opera.KptL1Cost', weight=70.0),
            oks_cost=dict(type='opera.OksCost', num_keypoints=16, weight=7.0))), # Change to 16 need to implement
